...
app = faust.App('windowed-agg', broker=KAFKA, version=1, topic_partitions=3)
...
def window_processor(key, events):
    ...
    sink.send_soon(value=AggModel(date=timestamp, count=count, mean=mean, values=values))
...
tumbling_table = (
    ...
    .hopping(WINDOW, 3, expires=timedelta(seconds=WINDOW_EXPIRES), key_index=True)
...
async def print_windowed_events(stream):
    async for event in stream:
        ...
        print(list(tumbling_table.values()))


@app.agent(sink)
async def windowed_events(stream):
    async for event in stream:
        print("event -> from sink:", event)
